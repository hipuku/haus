...
Or walk a whole page, using the same function with no `root`:
...
| `borderColor` | four sides | only sides with non-zero width, because computed border colour defaults to `currentColor` even at width `0` |
