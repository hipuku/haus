...
const shadows = [
  ...
  { name: 'shadow-sm',    css: 'var(--haus-shadow-sm)',    use: 'Subtle card lift, form inputs' },
  ...
  if (name === 'shadow-none') {
    return (
      <div style={{ width: 112, height: 72, flexShrink: 0,
        ...
        border: '1px dashed var(--haus-color-border-strong)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        <span aria-hidden="true" style={{ fontFamily: 'var(--haus-font-mono)', fontSize: '0.625rem', color: 'var(--haus-color-ink-secondary)' }}>flat</span>
      </div>
    )
  }
  return (
  ...
  return (
    <div style={pageWrap}>
      ...
      <p style={pageDescStyle}>
        Shadows are tinted with damson H=290° so they read as depth, not dirt. The focus ring is a two-layer shadow: an inner ring matches the surface colour to punch a visible gap, then the outer ring carries the focus signal via <code style={{ fontFamily: 'var(--haus-font-mono)' }}>--color-border-focus</code>. Demos on a sunken backdrop so shadow-sm is legible.
      </p>
      ...
      <TableGrid columns={ELEV_COLS}>
        ...
        {elevationRoles.map(({ name, alias, use }) => (
          <React.Fragment key={name}>
            ...
            <TD style={{ padding: 'var(--haus-space-3) 0' }}><span style={{ fontSize: 'var(--haus-type-body-sm-size)', color: 'var(--haus-color-ink-secondary)', whiteSpace: 'nowrap' }}>{use}</span></TD>
          </React.Fragment>
        ))}
      </TableGrid>
    </div>
...
function ZIndexPage() {
  return (
    <div style={pageWrap}>
      ...
      <p style={pageDescStyle}>
        Fixed stacking order prevents z-index inflation. Bands are intentionally spaced: a sticky header at 200 and its dropdown at 100 coexist without conflict. Reference via <code style={{ fontFamily: 'var(--haus-font-mono)' }}>var(--haus-z-modal)</code> in CSS or <code style={{ fontFamily: 'var(--haus-font-mono)' }}>tokens.zIndex</code> in JS. Never place a component outside its correct band.
      </p>
...
function OpacityPage() {
  return (
    <div style={pageWrap}>
      ...
      <p style={pageDescStyle}>
        Two values for two jobs. <code style={{ fontFamily: 'var(--haus-font-mono)' }}>opacity-disabled</code> (0.4) for unavailable elements, visible but non-interactive. <code style={{ fontFamily: 'var(--haus-font-mono)' }}>opacity-overlay</code> (0.6) for modal scrims. Never simulate colour tints with opacity. Use the palette's subtle and border steps instead.
      </p>
      ...
      <TableGrid columns={OP_COLS}>
        ...
        {opacities.map(({ name, value, use }) => (
          <React.Fragment key={name}>
            <TD>
              <div style={{ display: 'flex', gap: 'var(--haus-space-1)', flexShrink: 0 }}>
                <div style={{ width: 22, height: 28, background: 'var(--haus-color-primary-default)', borderRadius: 'var(--haus-radius-sm)', opacity: parseFloat(value), flexShrink: 0 }} />
                <div style={{ width: 22, height: 28, background: 'var(--haus-color-success-default)', borderRadius: 'var(--haus-radius-sm)', opacity: parseFloat(value), flexShrink: 0 }} />
              </div>
            </TD>
            <TD><CopyChip name={name} /></TD>
            <TD><span style={{ ...tokenValueStyle, whiteSpace: 'nowrap' }}>{value}</span></TD>
