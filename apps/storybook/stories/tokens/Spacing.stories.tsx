...
function SpaceRolesPage() {
  return (
    <div style={pageWrap}>
      ...
      <p style={pageDescStyle}>
        All three families run the same twelve steps. They did not until haus#26: <code style={{ fontFamily: 'var(--haus-font-mono)' }}>inset</code> had seven, <code style={{ fontFamily: 'var(--haus-font-mono)' }}>gap</code> four and <code style={{ fontFamily: 'var(--haus-font-mono)' }}>stack</code> two, and five of the twelve primitives were reachable by no role at all, so a product wanting 28px of padding had to read past this layer, which is the tier break the build fails over one layer in. The ladder came from drift, step for step, because drift had worked its own out in practice; drift took its tokens in-house in 4.0.0, and the ladder stayed.
      </p>
