...
import { Listbox } from 'haus-components'
...
function Controlled(props: Partial<React.ComponentProps<typeof Listbox>>) {
    ...
    docs: {
      description: {
        component:
          ...
          '\n\n' +
          '**It does not replace `Select`, and both stay** (decision 0020). ' +
          'Promoted on evidence (haus#61): haus already shipped a native ' +
          '`Select` that **no consumer imports**, while two products built this ' +
          'and wrote down the same reason. core: *"a styled listbox standing in ' +
          "for `<select>`, whose native popup can't be themed\"*. vault: *\"a " +
...
export const LabelOnly: Story = {
  render: () => {
    function Plain() {
      ...
      return <Listbox options={plain} value={value} onChange={setValue} label="Sort" />
    }
    return <Plain />
  },
  parameters: {
...
export const Sizes: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "The trigger is deliberately the same geometry as `Select`'s closed " +
          'control, so a product swapping one for the other sees nothing move.',
      },
    },
  },
}
